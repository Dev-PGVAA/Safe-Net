...
const topicCards = [
  { name: "Фишинг", icon: "🎣", color: "from-orange-500 to-red-600" },
  { name: "Пароли", icon: "🔐", color: "from-emerald-500 to-teal-600" },
  { name: "Соцсети", icon: "💬", color: "from-blue-500 to-cyan-600" },
  { name: "Покупки", icon: "🛍️", color: "from-purple-500 to-pink-600" }
];
            ...
            <div className="relative bg-slate-800 rounded-2xl p-6 border border-slate-700">
              <div className="grid grid-cols-2 gap-4">
                {topicCards.map((item, index) => (
                  ...
                  >
                    <div className={`w-12 h-12 bg-gradient-to-br ${item.color} rounded-lg flex items-center justify-center text-2xl mb-3 group-hover:scale-110 transition-transform`}>
                      {item.icon}
                    </div>
                    <div className="font-semibold text-white text-sm">{item.name}</div>
                    <div className="text-xs text-slate-400 mt-1">Изучить тему</div>
                  </div>
                ))}
              </div>
            </div>

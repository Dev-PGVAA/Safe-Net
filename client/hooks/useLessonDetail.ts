...
export function useLessonDetail() {
	...
	const {
		...
		queryFn: async () => {
			...
			return {
				...
				estimatedDuration: data.estimatedDuration || 15, // ✅ Используем реальное значение с сервера
			} as LessonWithNav
		},
		enabled: !!lessonId,
	})
	...
	const answerTask = (taskId: string, selectedOptionIds: string[]) => {
		answerMutation.mutate({ taskId, selectedOptionIds })
	}
	...
	return {
		...
		completedTasks,
		progress,
		estimatedDuration: lesson?.estimatedDuration || 1080, // ✅ Добавлено в return
		// Работа с ответами
		answerTask,

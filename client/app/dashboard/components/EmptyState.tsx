...
export const EmptyState = memo(
	({ icon, title, description, actionLabel, actionHref }: any) => {
		return (
			<div className='col-span-full relative rounded-[28px] overflow-hidden border border-dashed border-slate-700/50 bg-linear-to-br from-slate-900/40 via-slate-800/20 to-slate-900/40 backdrop-blur-xl p-10 md:p-14 text-center hover:border-slate-600/60 transition-all duration-1000 ease-out group'>
				...
				<div className='relative z-10'>
					...
					<h3 className='text-xl md:text-2xl font-bold text-white mb-4 tracking-tight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-linear-to-r group-hover:from-indigo-400 group-hover:to-purple-400 transition-all duration-700 ease-out'>
						{title}
					</h3>
					...
					{actionLabel && actionHref && (
						<Button
							variant='default'
							className='relative bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white shadow-lg hover:shadow-[0_20px_50px_-15px_rgba(99,102,241,0.4)] transition-all duration-700 ease-out hover:scale-105 border-0 overflow-hidden group/btn rounded-[14px] px-6 py-3 font-semibold'
							asChild
						>
							<Link
								href={actionHref}
